...
class MessageForTranslateDTO(Schema):
    ...
    translated_message = fields.Str(required=True, validate=validate.Length(min=1))
    message_date = fields.Date(required=True, validate=validate.Range(min=datetime.now()))
    source_language = fields.Str(required=True, validate=validate.Length(min=1))
